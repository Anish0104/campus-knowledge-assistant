...
def extract_content(html: str) -> tuple[str, str, str]:
    ...
    return title, text + "\n", selected_selector
    """Extract readable blocks without flattening inline words."""
    soup = BeautifulSoup(html, "html.parser")

    heading = soup.find("h1")
    title_element = heading or soup.title

    if title_element is None:
        raise ValueError("No page title found. Review this page manually.")
    ...
    if root is None:
        raise ValueError(
            "No main-content region found. "
            "A page-specific selector is needed."
        )
    ...
    for element in root.find_all(list(block_names)):
        # Avoid including both an outer list item and its nested paragraph.
        if any(
            parent.name in block_names
            for parent in element.parents
            if parent is not root
        ):
            continue
    ...
    # Basic extraction checks, not proof of completeness.
    if len(text.split()) < 40:
        raise ValueError(
            "Extracted fewer than 40 words. Review the page manually."
        )

    if len(text) > 100_000:
        raise ValueError(
            "Unexpectedly large extraction. Check the content selector."
        )
